...
const Home = () => {
    ...
    // нгазвание функции берется из смарт-контракта
    ...
    return (
        <div className="bg-[#091B18] min-h-screen flex flex-col">
            ...
            <div className={"flex-1"}>
                <Header/>
                <Marquee className={"bg-[#0A1F1C] p-5 md-5"} gradient={false} speed={100}>
                    <div className={"flex space-x-2 mx-10"}>
                        ...
                        <h4 className={"text-white font-bold"}>Previous winnings: {" "}
                            {lastWinnerAmount && ethers.utils.formatEther(lastWinnerAmount?.toString())} {currency}
                        </h4>
                    </div>
                </Marquee>
                ...
                {/*The Next Draw box*/}
                <div
                    className={"space-y-5 md:space-y-0 m-5 md:flex md:flex-row items-start justify-center md:space-x-5"}>
                    <div className={"stats-container"}>
                        ...
                        <div className={"flex justify-between p-2 space-x-2"}>
                            <div className={"stats"}>
                                ...
                                <p className={"text-xl"}>
                                    {/*Форматируем при помощи ethers валюту*/}
                    ...
                    <div className={"stats-container space-y-2"}>
                        <div className={"stats"}>
                            ...
                            <div
                                className={"flex text-white items-center space-x-2 bg-[#091B18] border-[#004337] border p-4"}>
                                <p>TICKETS</p>
                                <input
                            ...
                            <div className={"space-y-2 mt-5"}>
                                <div
                                    className={"flex items-center justify-between text-emerald-300 text-sm italic font-extrabold"}>
                                    <p>Total cost of tickets</p>
                                    <p>
                                        {ticketPrice && Number(ethers.utils.formatEther(ticketCommission.toString())) * quantity} {currency}
                                    </p>
                                </div>
                            ...
                            <button
                                ...
                                className={"mt-5 w-full bg-gradient-to-br " +
                                    "from-orange-500 to-emerald-600 px-10 py-5 rounded-md font-semibold text-white shadow-xl " +
                                    "disabled:from-gray-600 disabled:to-gray-100 disabled:to-gray-600 disabled:cursor-not-allowed"}>
                                Buy {quantity} tickets
                                for {ticketPrice && Number(ethers.utils.formatEther(ticketPrice.toString())) * quantity} {currency}
                            </button>
                        </div>
                        {userTickets > 0 && (
                            <div className={"stats"}>
                                    ...
                                    {Array(userTickets).fill("").map((_, index) => (
                                        <p key={index}
                                           className={"text-emerald-300 h-20 w-12 " +
                                               "bg-emerald-500/30 rounded-lg flex " +
            ...
            <footer className={"border-t border-emerald-500/20 flex items-center text-white justify-between p-5"}>
                ...
                <p className={"text-xs text-emerald-900 pl-5"}>
                    DISCLAMER: The information that exists on this site is strictly meant for informational and
                    entertainment purposes only. Any of this site’s content is the sole opinion of this site.
                    Everything posted on this site is meant to be as accurate and current as possible. However,
                    there are no expressed or implicit warranties nor representations with regard to the content’s
                    accuracy, completeness, and/or reliability.
                </p>
            </footer>
        </div>
    )
